...
class Lock {
  public:
    ...
    bool value;   //lock value
  private:
    char* name;	 // for debugging
    
    List *lockWaitQueue; //threads waiting in Acquire() for the value to be true(Free)
    Thread *lockOwner; //owner of the lock
};
...
class Condition {
  public:
    ...
    void Broadcast(Lock *conditionLock);// the currentThread for all of 
					// these operations
	Lock* getWaitingLock(); //new for Project 2; used while removing a CV from list of kernel CVs (destroyCV_Syscall)

  private:
    char* name;
    Lock *waitingLock; //waiting threads
    List *cvWaitQueue;
